...
if (!function_exists('usp_display_options_page')) {
	function usp_display_import_export() { 
		...
		$display  = '<p class="toggle-intro">'. __('Here you can download a backup of your settings, and/or import previous settings as needed. Note that your USP Pro License Key is not saved in the backup file.', 'usp') .'</p>';
		
		$display .= '<h4>'. __('Backup/Export', 'usp') .'</h4>';
		$display .= '<form method="post">';
		$display .= '<p>'. __('Download your current settings:', 'usp') .'</p>';
		$display .= '<p><input type="submit" name="submit" id="submit" class="button" value="'. __('Export Settings', 'usp') .'"></p>';
		...
		$display .= '<h4>'. __('Restore/Import', 'usp') .'</h4>';
		$display .= '<form method="post" enctype="multipart/form-data">';
		$display .= '<p>'. __('Step 1: Select the backup file that you would like to restore:', 'usp') .'</p>';
		$display .= '<p><input type="file" name="file_select" /></p>';
		$display .= '<p>'. __('Step 2: Click to restore your settings:', 'usp') .'</p>';
		$display .= '<p><input type="submit" name="submit" id="submit" class="button" value="'. __('Import Settings', 'usp') .'"></p>';
