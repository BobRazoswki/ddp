...
function callback_input_text_label($id) {
	...
	elseif ($id == 'usp_error_18')            $label = __('Name for the &ldquo;Agree to Terms&rdquo; field.', 'usp') .' <a target="_blank" href="https://plugin-planet.com/usp-pro-shortcodes/#agree-to-terms">'. __('Learn more&nbsp;&raquo;', 'usp') .'</a>';
	...
	elseif ($id == 'scheduled_subject_admin') $label = __('Subject line for scheduled alerts sent to the admin', 'usp');
	elseif ($id == 'scheduled_subject')       $label = __('Subject line for scheduled alerts sent to the user', 'usp');
	...
	elseif ($id == 'cc_scheduled')            $label = __('Additional addresses for scheduled alerts (comma-separated)', 'usp');
	elseif ($id == 'character_min')           $label = __('Minimum number of characters required for post content field (0 = no minimum)', 'usp');
	elseif ($id == 'character_max')           $label = __('Maximum number of characters allowed for post content field (0 = no maximum)', 'usp');
	...
	elseif ($id == 'tags_number')             $label = __('Number of tags that should be displayed for the &ldquo;Post Tags&rdquo; setting (use', 'usp') .' <code>-1</code> '. __('to display all tags)', 'usp');
...
function callback_textarea_label($id) {
	...
	elseif ($id == 'scheduled_message_admin') $label = __('Message for scheduled alerts sent to the admin', 'usp');
	elseif ($id == 'scheduled_message')       $label = __('Message for scheduled alerts sent to the user', 'usp');
	...
	elseif ($id == 'style_simple')            $label = __('CSS for Simple form style (edit as needed to fit your theme)', 'usp');
	...
	elseif ($id == 'usp_error_18_desc')       $label = __('"Agree to Terms" errors &ndash; when using', 'usp') .' <code>[usp_agree]</code> '. __('shortcode', 'usp');
	...
	elseif ($id == 'file_square')             $label = __('Image is not square (width does not equal height)', 'usp');
...
function callback_checkbox_label($id) {
	...
	elseif ($id == 'send_scheduled_admin') $label = __('Send alert to the admin when a submitted post is scheduled', 'usp');
	elseif ($id == 'send_scheduled_user')  $label = __('Send alert to the user when a submitted post is scheduled', 'usp');
	...
	elseif ($id == 'cats_nested')          $label = __('Enable nested/hierarchical display of subcategories', 'usp');
	...
	elseif ($id == 'square_image')         $label = __('Require each submitted image to have an equal width and height', 'usp');
